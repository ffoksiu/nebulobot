...
function updateBotStatus() {
    ...
    if (!statusData) {
        ...
        currentStatusIndex = (currentStatusIndex + 1) % (config.main.bot_statuses.length || 1);
    ...
    if (activityTypeMap[statusData.activity_type] === undefined) { 
...
client.once('ready', async () => {
    ...
    const adminCommandConfig = config.commands_deployment.find(cmd => cmd.module_name === 'admin');
    const adminBaseCommandName = adminCommandConfig ? adminCommandConfig.base_command_name : 'config';
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        logger.debug(`[DEBUG] Inspecting commandModule for '${interaction.commandName}':`);
        logger.debug(`[DEBUG] Type of commandModule: ${typeof commandModule}`);
        logger.debug(`[DEBUG] Is handleInteraction a function? ${typeof commandModule.handleInteraction}`);

        if (interaction.commandName === adminBaseCommandName && interaction.options.getSubcommand() === 'reload') {
            try {
                loadConfig();
                await interaction.reply({ content: config.admin.messages.config_reload_success, ephemeral: true });
                logger.info('Configuration reloaded from file by admin command.');
            } catch (reloadError) {
                logger.error(`Error reloading config via command: ${reloadError.message}`);
                await interaction.reply({ content: config.admin.messages.config_reload_fail.replace('{error}', reloadError.message), ephemeral: true });
            }
            return;
        }
    ...
    if (interaction.isButton() || interaction.isModalSubmit() || interaction.isStringSelectMenu()) {
        if (interaction.customId.startsWith('config_') || interaction.customId.startsWith('modal_config_')) {
            ...
            if (adminCommandModule && adminCommandModule.handleConfigInteraction) {
                try {
                    await adminCommandModule.handleConfigInteraction(interaction, db_pool, config);
                } catch (error) {
                    logger.error(`Error executing admin config interaction ${interaction.customId}: ${error.message}`);
                    logger.debug(error.stack);
                    if (interaction.replied || interaction.deferred) {
                        await interaction.followUp({ content: 'There was an error while processing this configuration action!', ephemeral: true });
                    } else {
                        await interaction.reply({ content: 'There was an error while processing this configuration action!', ephemeral: true });
                    }
                }
            }
        }
