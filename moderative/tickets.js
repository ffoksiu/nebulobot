...
module.exports = {
    ...
    closeTicket: async (guildId, channelId, closerId, db_pool, config) => {
        ...
        try {
            ...
            if (ticketChannel) {
                const fetchedMessages = await ticketChannel.messages.fetch({ limit: 500 });
    ...
    setTicketStatus: async (guildId, channelId, newStatus, setterId, db_pool, config) => {
        const messages = config.tickets.messages;
        const validStatuses = Object.keys(config.tickets.ticket_statuses).filter(s => s !== 'Closed');
    ...
    setupTicketPanel: async (client, guildId, panelChannelId, categoryId, transcriptsChannelId, db_pool, config) => {
        const messages = config.tickets.messages;
        const tickets_config_table_name = `tickets_config_${guildId}`;
        let connection;
        try {
