...
module.exports = {
    ...
    handleInteraction: async (interaction, db_pool, config) => {
        ...
        const isManagement = member.permissions.has(PermissionFlagsBits.Administrator) || member.roles.cache.some(role => ticketsConfig.management_role_ids.includes(role.id));
        ...
        if (subcommand === 'close') {
            ...
            if (isTicketCreator || isManagement) {
                ...
                const result = await ticketsModule.closeTicket(guildId, channelId, userId, db_pool, config);
                if (result.success) {
            ...
            ticketsModule.pendingCloseConfirmations.set(channelId, { interaction: confirmMessage, closerId: userId, timeout: timeoutId });
